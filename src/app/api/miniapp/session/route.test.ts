...
const auth = vi.hoisted(() => ({ createBearerSession: vi.fn(() => "user-1.sig"), createMiniappBindingToken: vi.fn(() => "bind.openid.sig"), verifyMiniappBindingToken: vi.fn(() => "openid-1") }));
...
describe("mini-program session adapter", () => {
  ...
  it("requires a valid bearer session and matching existing email for binding", async () => {
    const response = await PUT(new Request("http://localhost/api/miniapp/session", { method: "PUT", body: JSON.stringify({ bindingToken: "bind.openid.sig", email: "owner@example.com" }) }));
